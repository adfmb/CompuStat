...
numerador<-reactive(sapply(x(),rate=m(),FUN=dexp))
#numerador<-sapply(x,rate=m,FUN=dexp)
#numerador1<-sapply(x,m,FUN=densidadExp)

denominador<-reactive(sapply(x(),lambda=lg(),FUN=g_x))
#denominador<-sapply(x,lambda=m,FUN=g_x)

zetha<-reactive(qnorm(input$alpha/2,lower.tail = FALSE))
cociente<-reactive(numeric(length(denominador())))
medias_muestrales<-reactive(numeric(length(denominador())))
...
for(i in 1:length(denominador())){
  ...
  n1<-reactive(as.numeric(unlist(numerador()[i])))
  d1<-reactive(as.numeric(unlist(denominador()[i])))
  #cociente[i]<-n1/d1
  medias_muestrales()[i]<-capture(i)(reactive(mean(n1/d1)))
...
tabla<-reactive(data.frame(N=N(),Estimador=medias_muestrales,Limite_Inferior=liminf,Limite_Superior=limsup))
